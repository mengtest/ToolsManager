...
- (void) restorePurchas
{
	UnitySendMessage("DWSDKRoot", "OCLog", "restorePurchas");
    [[SKPaymentQueue defaultQueue] restoreCompletedTransactions];
}

- (void) paymentQueueRestoreCompletedTransactionsFinished:(SKPaymentQueue *)queue
{
    UnitySendMessage("DWSDKRoot", "OCLog", "restorePurchas Success");
...
{
	UnitySendMessage("DWSDKRoot", "OCLog", "restorePurchas Fail");
}
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response
{
	...
	if (response.products == nil || response.products.count == 0) {
        UnitySendMessage("DWSDKRoot", "purchaseFailed", "");
    }
	else
	{
        ...
        for(SKProduct *item in response.products)
		{
            ...
            if(array != nil && array.count >= 2)
            {
                ...
                {
                    UnitySendMessage("DWSDKRoot", "BuyIAPFailUnlegalCountry","");
                    break;
                }
            }
...
//# get good list fail
 ...
 - (void)request:(SKRequest *)request didFailWithError:(NSError *)error
 {
    ...
    UnitySendMessage("DWSDKRoot", "purchaseFailed", [errorStr UTF8String]);
 }
...
//
- (void)BuyIapWithID:(NSString *) m_id
{
	[[SKPaymentQueue defaultQueue] addTransactionObserver:self];	
...
{
	UnitySendMessage("DWSDKRoot", "OCLog", "paymentQueue");
    for (SKPaymentTransaction *transaction in transactions)
    {
        ...
        switch (transaction.transactionState)
        {
            case SKPaymentTransactionStatePurchased:
				UnitySendMessage("DWSDKRoot", "OCLog", "paymentQueue SKPaymentTransactionStatePurchased");
                [self completeTransaction:transaction];
                break;
            case SKPaymentTransactionStateFailed:
				UnitySendMessage("DWSDKRoot", "OCLog", "paymentQueue SKPaymentTransactionStateFailed");
                [self failedTransaction:transaction];
                break;
            case SKPaymentTransactionStateRestored:
				UnitySendMessage("DWSDKRoot", "OCLog", "paymentQueue SKPaymentTransactionStateRestored");
				[self completeTransaction:transaction];
                break;
            default:
                break;
...
{
	UnitySendMessage("DWSDKRoot", "OCLog", "Ready PurchaseSucceedWithPid");
    ...
    {  
		if([pid isEqualToString:transaction.payment.productIdentifier] && [tid isEqualToString:transaction.transactionIdentifier])
		{
			...
			UnitySendMessage("DWSDKRoot", "OCLog", "PurchaseSucceedWithPid Success");		 
 ...
 - (void)completeTransaction:(SKPaymentTransaction *)transaction
 {
     ...
     UnitySendMessage("DWSDKRoot", "BuyIAPSuccess",[[NSString stringWithFormat:@"%@,%@,%@",transaction.payment.productIdentifier,transaction.transactionIdentifier,jsonobjectstring] UTF8String]);
 ...
 - (void)failedTransaction:(SKPaymentTransaction *)transaction
{
	...
	UnitySendMessage("DWSDKRoot", "BuyIAPFail", [[NSString stringWithFormat:@"%d",transaction.error.code] UTF8String]);
	[[SKPaymentQueue defaultQueue] finishTransaction: transaction];
}
- (void) dealloc
{
