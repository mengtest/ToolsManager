...
namespace FMODUnity
{
    ...
    public class RuntimeManager : MonoBehaviour
    {
        ...
        public static void LoadBank(string bankName, bool loadSamples = false)
        {
            ...
            else
            {
                ...
                FMOD.RESULT loadResult = FMOD.RESULT.ERR_INVALID_PARAM;
                #if UNITY_ANDROID && !UNITY_EDITOR
                if (!bankPath.StartsWith("file:///android_asset")&& !bankPath.Contains("/storage/"))
                {
                ...
                else
                {
					...
					Debug.LogError("LoadBank Error:" + bankPath + ", result:" + loadResult.ToString());
                }
            }
        }
	...
	public static void LoadBank(string bankName, string bankPath, bool loadSamples = false)
	{
		...
		else
		{
			...
			{
				//throw new BankLoadException(bankPath, loadResult);
				Debug.LogError("LoadBank Error:" + bankPath + ", result:" + loadResult);
			}
		}
	}
        ...
        public static Guid PathToGUID(string path)
        {
            ...
            else
            {
                if (Instance.studioSystem != null)
                {
                    ...
                    {
                        Debug.LogError("FMOD.RESULT.ERR_EVENT_NOTFOUND:" + path);
                    }
                }
            }
            return guid;
        ...
        public static FMOD.Studio.EventInstance CreateInstance(Guid guid)
        {
            ...
            if (is3D)
            {
                // Set position to float_max, set3DAttributes should be called by the dev after this.
                newInstance.set3DAttributes(RuntimeUtils.To3DAttributes(new Vector3(float.MaxValue, float.MaxValue, float.MaxValue)));
            ...
            else
            {
                if (Instance.studioSystem != null)
                {
                    ...
                    if (result != FMOD.RESULT.OK)
                    {
                        ...
                        Debug.LogError("Event not found, guid:" + guid);
                    }
                }
        ...
        public static FMOD.Studio.VCA GetVCA(String path)
        {
            ...
            if (result != FMOD.RESULT.OK)
            {
				...
				Debug.LogError("VCANotFound:" + path);
            }
            return vca;
        }
