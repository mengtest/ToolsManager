...
function LuaNetWork.SetPlayID(play_id)
	_s.PlayID = play_id
end

function LuaNetWork.Register()
	--构造函数
...
function LuaNetWork.RegisterHandle(cmd, func, obj)
...
function LuaNetWork.UnRegisterHandle(cmd, func, obj)
	_UnRegisterHandle(cmd, func, obj)
end


function LuaNetWork.HandleMsg(netName,tcmd, tseq, areaID,errno, isSuc, data)
	...
	if tcmd ~= GAME_CMD.CS_HEART_BEAT and tcmd ~= GAME_CMD.SC_COMMON_RSP then
		DwDebug.LogWarning("LuaNetWork","LuaNetWork.HandleMsg: cmd = "..tcmd..";seq = "..tseq..";playid=".._s.PlayID, ";body", body)
	end
	...
	DwDebug.Log("LuaNetWork.HandleMsg: tcmd = "..tcmd.."seq = "..tseq)
	...
	if tcmd == ProtoManager.GetRspCMD(areaID) then
		...
		if head.errno ~= successCodeNum and body.eventId ~= ProtoManager.GetHeatBeatCMD(areaID) then
			...
			DwDebug.Log("LuaNetWork","LuaNetWork.HandleMsg: cmd = "..tcmd..";seq = "..tseq..";playid=".._s.PlayID, ";body", body)
			--通用服务端错误码事件
...
function LuaNetWork.CheckIsDisbandMessage(tipMsg)
...
local function _SendMsg(cmd, mbody, succesFunc, failedFunc,hasCb)	
	...
	if cmd ~= GAME_CMD.CS_HEART_BEAT then
		DwDebug.LogWarning("LuaNetWork","LuaNetWork.SendMsg: cmd = "..cmd.."seq = "..seq..";playid=".._s.PlayID, ";body", mbody)
	end
...
--		end
	...
	if data then
		sendFunc(_s.gameNetName,_s.PlayID,cmd, seq, hasCb, needLockScreen,data)--区域ID临时这样用，后面再优化
	else
		sendFunc(_s.gameNetName,_s.PlayID,cmd, seq, hasCb,needLockScreen)
	end
end
...
function LuaNetWork.SendMsg(cmd, mbody, succesFunc, failedFunc, isForce)
	DwDebug.Log("LuaNetWork.SendMsg: cmd = "..cmd .. tostring(m_logic_wait_call) .. tostring(isForce))	
	if m_logic_wait_call and not isForce then
		DwDebug.LogError("LuaNetWork.SendMsg: cmd = "..cmd .. " is abandon because send is in wait cd ")
		return
	end
	if not  isForce then
...
--- 重置清空标记
function  LuaNetWork.Reset()
	m_logic_wait_call = false
end

function LuaNetWork.UpdateSecond()
