local SensitiveWordUtils = {inited = false}

local utf8 = require("LuaWindow.Chat.utf8")
...
function SensitiveWordUtils:init()
    if self.inited then
        return
    end
    local words = require("Config.SensitiveWordCfg").getTotalCfg()
    -- local words = {[1] = "毛泽东"}
    ...
    self.inited = true
    -- package.loaded["ChatSensitiveCfg"] = nil
end

function SensitiveWordUtils:getFiltrationString(s)
...
-- 如果校验合法 返回true 校验失败范围false
function SensitiveWordUtils:checkString(s)
    if type(s) ~= "string" then
        return false
    end
    local i = 1
...
-- print(SensitiveWordUtils:getFiltrationString("aaa毛泽东"))
-- print(SensitiveWordUtils:checkString("aaa毛泽东"))
-- print(SensitiveWordUtils:checkString("1"))
