...
function yhmj.Init(_self)
    if not _self then
        DwDebug.Log("yhmj.Init error: _self")
        return
    end
    self = _self
...
function yhmj.InitList(self)
    ...
    yhmj.update_JiangMa(self, is_show_jiangMa)
    local players_data = self.uiData.playerMes
    for i = 1, 4 do
...
-- 奖马逻辑
function yhmj.update_JiangMa(self, is_show)
    local ctx = self.ctx
    ...
    yhmj.SwitchJiangMaUI(self, is_show)
end
-- 切换奖马相关节点的坐标变化
function yhmj.SwitchJiangMaUI(self, is_show)
    local ctx = self.ctx
...
function yhmj._InitItem(self, index, data)
    ...
    yhmj._GenMJNodes(self, node, data)
end
-- 生成麻将节点
function yhmj._GenMJNodes(self, node, data)
    local ctx = self.ctx
