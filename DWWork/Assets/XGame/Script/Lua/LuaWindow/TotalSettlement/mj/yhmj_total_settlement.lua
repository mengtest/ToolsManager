...
function yhmj.Init(_self)
    if not _self then
        DwDebug.Log("yhmj.Init total error: _self")
        return false
    end
    self = _self
    ...
    yhmj._list_item_fun_handler = {}
    yhmj._scrollviewObjs = {}
    return true
end
-- 初始化列表
function yhmj.InitList(self)
    local ctx = self.ctx
    ...
    for i = 1, 4 do
        yhmj._InitItem(self, i, players_data[i])
    end
...
-- 初始化玩家列表 子项
function yhmj._InitItem(self, index, data)
    local ctx = self.ctx
    ...
    local scrollviewObj = yhmj._scrollviewObjs[index]
    ...
    -- 列表逻辑
        ...
        yhmj._scrollviewObjs[index] = _scrollviewObj
        scrollviewObj = _scrollviewObj
        yhmj._list_item_fun_handler[#yhmj._list_item_fun_handler + 1] = item_fun_handler
    end
    scrollviewObj:InitItemCount(#(scrollview_data), true)
end
-- 清理
function yhmj.clean()
    local list = yhmj._list_item_fun_handler
