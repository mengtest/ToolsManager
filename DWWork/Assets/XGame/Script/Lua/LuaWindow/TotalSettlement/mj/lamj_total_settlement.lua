...
function lamj.Init(_self)
    if not _self then
        DwDebug.Log("lamj.Init total error: _self")
        return false
    end
    self = _self
    ...
    lamj._list_item_fun_handler = {}
    lamj._scrollviewObjs = {}
    return true
end
-- 初始化列表
...
function lamj._InitItem(self, index, data)
    ...
    local scrollviewObj = lamj._scrollviewObjs[index]
    ...
    -- 列表逻辑
        ...
        lamj._scrollviewObjs[index] = _scrollviewObj
        scrollviewObj = _scrollviewObj
        lamj._list_item_fun_handler[#lamj._list_item_fun_handler + 1] = item_fun_handler
    end
    scrollviewObj:InitItemCount(#(scrollview_data), true)
end
-- 清理
function lamj.clean()
    local list = lamj._list_item_fun_handler
