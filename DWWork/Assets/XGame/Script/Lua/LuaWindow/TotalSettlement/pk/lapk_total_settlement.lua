...
function lapk.Init(_self)
    if not _self then
        DwDebug.Log("lapk.Init total error: _self")
        return false
    end
    self = _self
    ...
    lapk._list_item_fun_handler = {}
    lapk._scrollviewObjs = {}
    return true
end
-- 初始化列表
...
function lapk._InitItem(self, index, data)
    ...
    local scrollviewObj = lapk._scrollviewObjs[index]
    ...
    -- 列表逻辑
        ...
        lapk._scrollviewObjs[index] = _scrollviewObj
        scrollviewObj = _scrollviewObj
        lapk._list_item_fun_handler[#lapk._list_item_fun_handler + 1] = item_fun_handler
    end
    scrollviewObj:InitItemCount(#(scrollview_data), true)
end
-- 清理
function lapk.clean()
    local list = lapk._list_item_fun_handler
