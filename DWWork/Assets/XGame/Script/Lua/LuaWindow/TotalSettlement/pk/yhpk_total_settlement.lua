...
function yhpk.Init(_self)
    if not _self then
        DwDebug.Log("yhpk.Init total error: _self")
        return false
    end
    self = _self
    ...
    yhpk._list_item_fun_handler = {}
    yhpk._scrollviewObjs = {}
    return true
end
-- 初始化列表
function yhpk.InitList(self)
    local ctx = self.ctx
    ...
    for i = 1, 4 do
        yhpk._InitItem(self, i, players_data[i])
    end
...
-- 初始化玩家列表 子项
function yhpk._InitItem(self, index, data)
    local ctx = self.ctx
    ...
    local scrollviewObj = yhpk._scrollviewObjs[index]
    ...
    -- 列表逻辑
        ...
        yhpk._scrollviewObjs[index] = _scrollviewObj
        scrollviewObj = _scrollviewObj
        yhpk._list_item_fun_handler[#yhpk._list_item_fun_handler + 1] = item_fun_handler
    end
    scrollviewObj:InitItemCount(#(scrollview_data), true)
end
-- 清理
function yhpk.clean()
    local list = yhpk._list_item_fun_handler
