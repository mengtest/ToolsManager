...
function pk32.Init(_self)
    if not _self then
        DwDebug.Log("pk32.Init total error: _self")
        return false
    end
    self = _self
    ...
    pk32._list_item_fun_handler = {}
    pk32._scrollviewObjs = {}
    return true
end
-- 初始化列表
function pk32.InitList(self)
    local ctx = self.ctx
    ...
    for i = 1, 4 do
        pk32._InitItem(self, i, players_data[i])
    end
...
-- 初始化玩家列表 子项
function pk32._InitItem(self, index, data)
    local ctx = self.ctx
    ...
    local scrollviewObj = pk32._scrollviewObjs[index]
    ...
    -- 列表逻辑
        ...
        pk32._scrollviewObjs[index] = _scrollviewObj
        scrollviewObj = _scrollviewObj
        pk32._list_item_fun_handler[#pk32._list_item_fun_handler + 1] = item_fun_handler
    end
    scrollviewObj:InitItemCount(#(scrollview_data), true)
end
-- 清理
function pk32.clean()
    local list = pk32._list_item_fun_handler
