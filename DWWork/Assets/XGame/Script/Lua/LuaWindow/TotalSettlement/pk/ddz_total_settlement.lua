...
function ddzpk.Init(_self)
    if not _self then
        DwDebug.Log("ddzpk.Init total error: _self")
        return false
    end
    self = _self
    ...
    ddzpk._list_item_fun_handler = {}
    ddzpk._scrollviewObjs = {}
    return true
end
-- 初始化列表
function ddzpk.InitList(self)
    local ctx = self.ctx
    ...
    for i = 1, 4 do
        ddzpk._InitItem(self, i, players_data[i])
    end
...
-- 初始化玩家列表 子项
function ddzpk._InitItem(self, index, data)
    local ctx = self.ctx
    ...
    local scrollviewObj = ddzpk._scrollviewObjs[index]
    ...
    -- 列表逻辑
        ...
        ddzpk._scrollviewObjs[index] = _scrollviewObj
        scrollviewObj = _scrollviewObj
        ddzpk._list_item_fun_handler[#ddzpk._list_item_fun_handler + 1] = item_fun_handler
    end
    scrollviewObj:InitItemCount(#(scrollview_data), true)
end
-- 清理
function ddzpk.clean()
    local list = ddzpk._list_item_fun_handler
