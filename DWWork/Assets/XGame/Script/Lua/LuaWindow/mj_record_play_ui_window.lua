--------------------------------------------------------------------------------
--   File     : mj_mj_record_play_ui_window.lua
--   author : guoliang
--   function   : 回放主UI
--   date     : 2017-10-25
...
require "Logic.UICtrlLogic.UIMJRecordHandCardCtrl"
...
mj_record_play_ui_window = {
}
local _s = mj_record_play_ui_window
_s.m_showBtnRoot = true
_s.m_timeCount = 0

function mj_record_play_ui_window.Init(LuaWindowRoot)
	m_luaWindowRoot = LuaWindowRoot
end

function mj_record_play_ui_window.InitWindow(open, state)
...
function mj_record_play_ui_window.CreateWindow()
	...
	m_UIRecordHandCardCtrl = UIMJRecordHandCardCtrl.New()
	m_UIRecordHandCardCtrl:Init(m_handCardRootTrans,m_luaWindowRoot)
...
function mj_record_play_ui_window.SetWaterMark()
	local waterConfig = 
	{
...
function mj_record_play_ui_window.InitWindowDetail()
	m_luaWindowRoot:ShowChild(m_luaWindowRoot:GetTrans("bg_root"), "bg_window_" .. LogicUtil.GetMJBgType(), true)
...
function mj_record_play_ui_window.RefreshBtns(eventId, p1, p2)
...
function mj_record_play_ui_window.GetSpeedSpriteName(speed)
	local spriteNum = speed
	return "replayLayer_text_"..spriteNum
end


function mj_record_play_ui_window.ChangeWindowShowState()
...
function mj_record_play_ui_window.RegisterEventHandle()
...
function mj_record_play_ui_window.UnRegisterEventHandle()
...
function mj_record_play_ui_window.RefreshPlayerHead(eventId,p1,p2)
	local seatPos = p1
	local seatInfo = p2
	m_UIPlayerHeadCtrl:RefreshPlayerHead(seatPos,seatInfo)
end

function mj_record_play_ui_window.PlaySceneInit(eventId,p1,p2)
	local isPlayNow = p1
	-- 刷新头像位置
...
--刷新房间轮数
function mj_record_play_ui_window.RefreshRoomRoundNum(eventId,p1,p2)
	local curRound = p1
	local totalRound = p2
...
function mj_record_play_ui_window.RefreshRecordPauseStatus(eventId,p1,p2)
	local isPause = p1
	m_isPause = isPause
	if m_isPause then
		m_luaWindowRoot:SetSprite(m_labelPauseTrans,"replayLayer_text_play")
...
--显示剩余牌数
function mj_record_play_ui_window.ShowRemainCardNum(eventId,p1,p2)
...
function mj_record_play_ui_window.HandleWidgetClick(gb)
...
function mj_record_play_ui_window.HandleClickLastRound()
	LuaEvent.AddEventNow(EEventType.RecordPlayLastRound)
end

function mj_record_play_ui_window.HandleClickReply()
	LuaEvent.AddEventNow(EEventType.RecordReplayCurRound)
end

function mj_record_play_ui_window.HandleClickPause()
...
function mj_record_play_ui_window.HandleClickSpeed()
...
function mj_record_play_ui_window.HandleClickNextRound()
	LuaEvent.AddEventNow(EEventType.RecordPlayNextRound)
end


function mj_record_play_ui_window.HandleClickQuit()
...
function mj_record_play_ui_window.HanldeClickBgBtn()
	_s.ShowPlayBtnRoot(not _s.m_showBtnRoot,true)
end

-- 回放按钮隐藏与显示
function mj_record_play_ui_window.ShowPlayBtnRoot(isFoword,needTweenAnim)
...
function mj_record_play_ui_window.UpdateSecond()
...
function mj_record_play_ui_window.UnRegister()
	m_luaWindowRoot = nil
end

function mj_record_play_ui_window.OnDestroy()
	_s.UnRegisterEventHandle()
	if m_UIPlayerHeadCtrl then
		m_UIPlayerHeadCtrl:Destroy()
		m_UIPlayerHeadCtrl = nil
	end
	if m_UIRecordHandCardCtrl then
		m_UIRecordHandCardCtrl:Destroy()
		m_UIRecordHandCardCtrl = nil
	end
