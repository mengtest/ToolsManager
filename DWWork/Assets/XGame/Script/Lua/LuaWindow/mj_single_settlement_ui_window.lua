...
local path = "LuaWindow.SingleSettlement.mj."
local modulePath = {
    [Enum_SubLogicType.CRMJ] = path .. "crmj_single_settlement",
    [Enum_SubLogicType.LAMJ] = path .. "lamj_single_settlement",
    [Enum_SubLogicType.YHMJ] = path .. "yhmj_single_settlement",
}
--[[文件域全局]]
mj_single_settlement_ui_window = {}
local self = mj_single_settlement_ui_window
-- [[重载]]
-- 初始化
function mj_single_settlement_ui_window.Init(ctx)
    self.ctx = ctx
end
-- @parma logicType 参数说明 int型 /10得到扑克类型 %10得到是否回放(非0就是回放)
-- 初始化窗口
function mj_single_settlement_ui_window.InitWindow(openState, openData)
    -- 初始化局部变量
    local ctx = self.ctx
...
-- 窗口创建时候
function mj_single_settlement_ui_window.CreateWindow()
...
-- 窗口销毁时候
function mj_single_settlement_ui_window.OnDestroy()
    self.m_TimeValue = nil
    self.ctx = nil
    LuaEvent.RemoveHandle(EEventType.ScreenShotEnd, self.ScreenShotEnd)
end
-- 截屏事件
function mj_single_settlement_ui_window.ScreenShotEnd()
    local ctx = self.ctx
    if ctx then
...
-- 倒计时事件
function mj_single_settlement_ui_window._counDownEvent()
...
-- 主界面
function mj_single_settlement_ui_window.InitMainUI()
    ...
    ctx:SetLabel(ctx:GetTrans("txt_settleTime"), uiData.settlementTime)
    ...
    ctx:SetLabel(ctx:GetTrans("txt_roundProgress"), uiData.currentGamepos .. "/" .. uiData.totalGameNum .. "r")
    -- 屏蔽倒计时
    ctx:SetActive(ctx:GetTrans("txt_restSeconds"), false)
...
-- 按钮事件分发
function mj_single_settlement_ui_window.HandleWidgetClick(go)
    local str = go.name
...
-- 分享截屏
function mj_single_settlement_ui_window._on_btn_share()
...
-- 返回按钮
function mj_single_settlement_ui_window._on_btn_back()
    PlayGameSys.QuitToMainCity()
end
-- 关闭按钮
function mj_single_settlement_ui_window._on_btn_close()
    self._goToTotalSettlement()
    self._normalCloseLogic()
end
-- 继续按钮
function mj_single_settlement_ui_window._on_btn_continue()
    --是否回放
    if not self.isReplayMode then
...
-- 总结算按钮
function mj_single_settlement_ui_window._on_btn_totalSettlement()
    self._goToTotalSettlement()
...
-- 关闭房间相关逻辑
-- 刷新积分 更新对战流水 更新状态
function mj_single_settlement_ui_window._normalCloseLogic()
    -- local uiData = self.uiData
    ...
    --         LuaEvent.AddEvent(EEventType.RefreshPlayerTotalScore, player.seatPos, player.zongJifen)
    --     end
    -- end
...
-- 状态机切换
function mj_single_settlement_ui_window._goToTotalSettlement()
    local roomObj = self.roomObj
    if roomObj:GetBigResult() then
        roomObj:ChangeState(RoomStateEnum.GameOver)
    else
        if self.uiData.currentGamepos >= self.uiData.totalGameNum then
            WindowUtil.LuaShowTips("总结算信息准备中,请稍候...")
        end
    end
end
-- 添加到清理列表中去
function mj_single_settlement_ui_window.AddToCleanList(trans_table)
...
-- 清理生成的麻将节点
function mj_single_settlement_ui_window._doCleanList()
    local list = self.cleanList
    local ctx = self.ctx
