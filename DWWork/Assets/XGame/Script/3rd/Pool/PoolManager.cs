...
//		public Func<IObjectPool<T>> CommonPoolGenerateFunc<T> {get;set;}
		...
		private IObjectPool CreateCommonPool(Type t)
		{
			...
			if (typeof(IPoolable).IsAssignableFrom(t))
			{
				UnityEngine.Debug.Log("type:" + t.ToString() + " implements `IPoolable` interface, create a ResetablePool");

				var genericPoolType = typeof(ResettableObjectPool<>);
			...
			{
				UnityEngine.Debug.Log("type:" + t.ToString() + " not implements `IPoolable` interface, create a normal Pool");
		...
		{
			return CreateCommonPool(typeof(T));
		}
