...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		public PBXDictionary AddFile( string filePath, PBXGroup parent = null, string tree = "SOURCE_ROOT", 
		                             bool createBuildFiles = true, bool weak = false, bool needRemove= true)
		{
			...
			if( !( File.Exists( absPath ) || Directory.Exists( absPath ) ) && tree.CompareTo( "SDKROOT" ) != 0 ) {
				...
				if (filePath.Length > 4 && needRemove)
				{
			...
			Debug.Log("fileReference filepath = " + filePath);
		...
		{	
			...
			foreach( string filePath in mod.files ) {
				Debug.Log( "mod.path = " + mod.path + ", filePath = " + filePath );
				string absoluteFilePath = System.IO.Path.Combine( mod.path, filePath );
				Debug.Log( "absoluteFilePath = " + absoluteFilePath );
				this.AddFile( absoluteFilePath, modGroup );
			}
			//需要移动的文件
			if (mod.movefolders != null)
			{
				Debug.Log ("movefolders.." + mod.group);
				CreateFolder(projectRootPath, mod.group);
				ArrayList folderList = mod.movefolders;
				foreach (string filePath in folderList)
				{
					string direabsoluteFilePath = System.IO.Path.Combine(mod.path, filePath);
					CopyFolder(direabsoluteFilePath, projectRootPath + "/" + mod.group);
					DirectoryInfo dirInfo = new DirectoryInfo(direabsoluteFilePath);
                    Debug.LogError("direabsoluteFilePath: " + direabsoluteFilePath);
                    Debug.LogError("projectRootPath group: " + projectRootPath + "/" + mod.group);
					...
					for(int m = 0; m < needFileList.Count; m++)
					{
						...
						Debug.Log("nextFile.fullName = " + nextFile.FullName);
						...
						Debug.Log("nextFile.fullName = " + nextFile.FullName);
						//Debug.Log("mod.path = " + mod.path + ", filePath = " + filePath);
						string absoluteFilePath = nextFile.FullName;
						
						//DirectoryInfo info = new DirectoryInfo(projectRootPath + mod.group);
						
						string modPath = projectRootPath + "/" + mod.group;
						
						//FileInfo info = new FileInfo(absoluteFilePath);
						
						string moveToProjPath = modPath  + absoluteFilePath.Substring(absoluteFilePath.LastIndexOf("/"));
						
						this.AddFile(moveToProjPath, modGroup);
						
					}
					foreach(DirectoryInfo directInfo in dirInfo.GetDirectories())
					{
						this.AddFile(projectRootPath + "/" + mod.group + "/" + directInfo.Name , modGroup, "SOURCE_ROOT",true,false,false);
					}
					
				}
				
			}
			if (mod.movefiles != null)
			{
				foreach (string filePath in mod.movefiles)
				{
					Debug.Log("mod.path = " + mod.path + ", filePath = " + filePath);
					string absoluteFilePath = System.IO.Path.Combine(mod.path, filePath);
					...
					//DirectoryInfo info = new DirectoryInfo(projectRootPath + mod.group);
					if (!Directory.Exists(projectRootPath + "/" + mod.group))
					{
						Debug.Log("CreateDirectory projectRootPath + path[i]" + projectRootPath + "/" + mod.group);
						Directory.CreateDirectory(projectRootPath + "/" + mod.group);
					}
					
					
					FileInfo info = new FileInfo(absoluteFilePath);
					
					string moveToProjPath = projectRootPath + "/" + mod.group  + filePath.Substring(filePath.LastIndexOf("/"));
					info.CopyTo(moveToProjPath, true);
					Debug.Log("absoluteFilePath = " + moveToProjPath);
					this.AddFile(moveToProjPath, modGroup, "SOURCE_ROOT", true, false, false);
				}
			}
		...
		private void CreateFolder(string parentpath, string dicectPath)
		{
			if (string.IsNullOrEmpty(dicectPath))
			{
				return;
			}
			
			string[] paths = dicectPath.Split('/');
		...
		private void CopyFolder(string src, string dst)
		{
			...
			for (int m = 0; m < needFileList.Count; m++)
			{
				if (needFileList[m].FullName.EndsWith(".meta"))
				{
					continue;
				}
				FileInfo nextFile = needFileList[m];
				string moveToProjPath = dst + "/"+ nextFile.Name;
				nextFile.CopyTo(moveToProjPath);
			}
			List<DirectoryInfo> needFolderList = new List<DirectoryInfo>(dirInfo.GetDirectories());
			for (int m = 0; m < needFolderList.Count; m++)
			{
				CopyFolder(needFolderList[m].FullName, dst + "/" + needFolderList[m].Name);
			}
		}
		...
		public void Save()
		{
            ...
            Debug.LogError("Save Path: " + projectPath);
