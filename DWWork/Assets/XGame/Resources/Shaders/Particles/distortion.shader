...
Shader "EZFun/Particles/distortion" {
	Properties{
		_NoiseTex("絮乱图", 2D) = "white" {} // 絮乱图
	_TintColor("Tint Color", Color) = (0.5,0.5,0.5,0.5)
		_AreaTex("区域图(Alpha)：白色为显示区域，透明为不显示区域", 2D) = "white" {} // 区域图
	_MoveSpeed("絮乱图移动速度", range(0,2)) = 1 // 絮乱图移动速度
		_MoveForce("絮乱图叠加后移动强度", range(0,0.5)) = 0.5 // 絮乱图叠加强度，多张运动纹理叠加后再相乘的系数
	}
		Category{
		// 【渲染队列】在透明物体前，类型为【透明】
...
#include "UnityCG.cginc"
	...
	half4 frag(v2f i) : COLOR
	{
		...
		i.uvgrab.x += ((offsetColor1.r + offsetColor2.r) - 1) * _MoveForce*_TintColor.a; // 叠加强度
		i.uvgrab.y += ((offsetColor1.g + offsetColor2.g) - 1) * _MoveForce*_TintColor.a;
