...
public class UIAtlasMaker : EditorWindow
{
	...
	static public bool UpdateTexture (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		if (System.IO.File.Exists(newPath))
		{
			...
			{
				DebugEz.LogError(newPath + " is not editable. Did you forget to do a check out?");
				return false;
			}
#endif
		...
		if (PackTextures(tex, sprites))
		{
			...
			if (newTexture)
			{
				if (tex == null) DebugEz.LogError("Failed to load the created atlas saved as " + newPath);
