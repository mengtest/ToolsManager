...
public class UITextureInspector : UIBasicSpriteEditor
{
	...
	protected override bool ShouldDrawProperties ()
	{
        ...
        // Mingrui Jiang cgg, 在这里备份一下texture名称
        // 打更新包的时候texture不会被打进去，会missing
        // 但是texture的名字已经记录下来了，运行游戏的时候，根据这个名字找到已经在游戏resources里面的texture
        Texture spTx = sp.objectReferenceValue as Texture;
        var txName = serializedObject.FindProperty("mTextureName");
        if (txName == null || spTx == null)
            return false;

        txName.stringValue = spTx.name;
