...
public class LuaScriptMgr
{
    ...
    {
        var enm = fileList.GetEnumerator();
        while(enm.MoveNext())
        {
            lua.DoFile(enm.Current, null);
        }
    ...
    public void InitLuaScriptCallBack()
    {
        updateFunc = GetLuaFunction("GameManager.Update");
        lateUpdateFunc = GetLuaFunction("GameManager.LateUpdate");
        fixedUpdateFunc = GetLuaFunction("GameManager.FixedUpdate");
    }
    ...
    public void Update()
    {
        if (updateFunc != null)
        {
            updateFunc.Call2Args(Time.deltaTime, Time.unscaledDeltaTime);
        }
    ...
    public void FixedUpdate()
    {
        ...
        {
            fixedUpdateFunc.Call1Args(Time.fixedDeltaTime);
        }
    }
    ...
    public object[] DoString(string str, string fileName)
    {
        return lua.DoString(str, fileName, null);
    }
    ...
    public byte[] Loader(string name, out int length)
    {
 //       UnityEngine.Debug.LogError("Loader:" + name);
        byte[] str = null;
        fileList.Add(name);

#if !LUA_ZIP
        bool isNeedCry = false;
        string path = null;
        if (name.StartsWith("Table."))
        {
            path = Util.LuaPath(name.Substring("Table.".Length), out isNeedCry, false, ".lbytes", "Table");
        }
        else
        {
            path = Util.LuaPath(name, out isNeedCry, !Constants.RUN_WITH_EN_LUA);
        }
        str = EZFunTools.ReadFile(path);
        if (str == null)
        {
            Debugger.LogError("load file name:" + name + " Error");
            length = 0;
            return null;
        }
        byte[] zdata = null;
        if (isNeedCry)
        {
            try
            {
                zdata = GlobalCrypto.Decrypte(str, out length);
            }
            catch (Exception ex)
            {
                length = 0;
                UnityEngine.Debug.LogError("lua file  decrypte error:" + name+
                    "\nex:" + ex);
            }
        }
        else
    ...
    public static void RegisterNewEnumLib(IntPtr L, string libName, string[] kes, int[] values)
    {
        CreateTable(L, libName);

        for (int i = 0; i < kes.Length; i++)
        {
            LuaDLL.lua_pushstring(L, kes[i]);
            LuaDLL.lua_pushinteger(L, values[i]);
            // PushEnum(L, enums[i].val);
            LuaDLL.lua_rawset(L, -3);
        }
    ...
    static bool CheckType(IntPtr L, LuaTypes luaType, Type t, int pos)
    {
        ...
        switch (luaType)
        {
            case LuaTypes.LUA_TNUMBER:
                return t.IsPrimitive && t != typeof(bool);//t.IsPrimitive; bool也是原始类型 --add by xiaolong
    ...
    public static int Xml_read (IntPtr L) 
    {
        ...
        TextAsset ta = ResourceMgr.Load(xml, typeof(TextAsset)) as TextAsset;
    ...
    */
    ...
    public static int IsEnumEquals(IntPtr L)
    {
        ...
        object arg1 = LuaScriptMgr.GetLuaObject(L, 2);
