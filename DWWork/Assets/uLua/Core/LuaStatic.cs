namespace LuaInterface
{
    ...
    public delegate byte[] ReadLuaFile(string name,  out int length);
	...
	public static class LuaStatic
	{
        ...
        static byte[] DefaultLoader(string name, out int  length)
        {
            byte[] str = null;
            length = 0;
		...
		public static int print(IntPtr L)
		{
			...
			for( int i = 1; i <= n; i++ ) 
			{
				...
				t = LuaDLL.lua_tostring(L, -1);
				//Debug.Log("LUA ybgg: " + t);
				if (t != null) {
					s += t;
				}
		...
		{            
            ...
            int outLength = 0;		
            byte[] text = Load(fileName, out outLength);
			...
			LuaDLL.luaL_loadbuffer(L, text, outLength, fileName);
		...
		{            
            ...
            if (LuaDLL.luaL_loadbuffer(L, text, outlength, fileName) == 0)
            {
