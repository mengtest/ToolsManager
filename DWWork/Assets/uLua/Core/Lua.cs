...
namespace LuaInterface
{
    ...
    public class LuaState : IDisposable
    {
        ...
        public LuaState()
        {
			...
			GCHandle handle = GCHandle.Alloc(translator, GCHandleType.Normal);
        ...
        {            
            ...
            int length = 0;
            byte[] bt = LuaStatic.Load(fileName, out length);
            ...
            if (LuaDLL.luaL_loadbuffer(L, bt, length, fileName) != 0)
            {
                ThrowExceptionFromError(oldTop);
            }
        ...
        {            
            ...
            // Load with Unity3D resources       
            int outlength = 0;     
            byte[] text = LuaStatic.Load(fileName, out outlength);
