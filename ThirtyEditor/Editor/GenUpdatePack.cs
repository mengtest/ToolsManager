...
public class GenUpdatePack : EditorWindow 
{
	...
	{
		if (inputPath.IsNull() || inputPath.Equals(""))
		{
			return null;
		}
        ...
        EZFunTools.FastZipCompress(inputPath, outPath);


        fi.md5 = EZFunTools.GenMD5(outPath);
	...
	static long GenFile(string inputPath, bool isCry)
	{
		...
		EZFunTools.FastZipCompress(inputPath);
		var fileInfo = new FileInfo(filePath);
		return fileInfo.Length;
	}
	...
	static void CryFile(string path)
	{
		...
		var b = EZFunTools.ReadFile(path);
